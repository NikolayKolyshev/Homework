...
R = (-log(1 - rand(800, 800))).^0.5; % генерация шума Рэлея
Noise = im2double(Image); % создание массива для изображения
Noise = Noise + R; % слияние изображения с шумом Рэлея
Noise = im2uint8(Noise); % возврат изображения к типу 8-bit

% создание рабочего окна и его настройка
figure('Name','Гистограмма шума Рэлея', 'NumberTitle','off');
ImgHist = histogram(Noise); % реализация гистограммы шума Рэлея

% //////////// РАБОТА С ДИРЕКТОРИЙ ПРОЕКТА ////////////
...
% //////////// ПРЕОБРАЗОВАНИЯ С ОДНОЙ ФИГУРОЙ ////////////
...
% //////////// ПРЕОБРАЗОВАНИЯ С ДВУМЯ ФИГУРАМИ ////////////
...
% //////////// РАБОТА С МОЕЙ ЛИЧНОЙ КАРТИНКОЙ (ФОНОМ) ////////////
...
R = (-log(1 - rand(800, 800))).^0.5; % генерируем шум Рэлея ещё раз
